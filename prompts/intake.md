...
用户选择后记录 `mode = self | colleague`，后续问题根据模式调整措辞。
...
**蒸馏自己模式**：
```
我来帮你创建你自己的数字分身 Skill。只需要回答 3 个问题，每个都可以跳过。
```
...
**蒸馏自己模式**：
```
给自己的数字分身起个名字？（花名、昵称或代号都行，多个字用 - 连接）
...
**蒸馏自己模式**：
```
用一句话描述你自己的基本信息——公司、职级、职位、性别，想到什么写什么，跳过也行。
...
**蒸馏自己模式**：
```
用一句话描述你自己的性格——MBTI、星座、个性特点、企业文化烙印，
别人怎么看你？你觉得自己是什么样的人？想到什么写什么，跳过也行。
（也可以跳过，从聊天记录中推断）

例：ENFP 双子座 数据驱动 百度味 总被说话太直但自己觉得是高效
```
